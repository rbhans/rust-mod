...
async fn client_can_interact_with_in_memory_simulator_service() {
    ...
    let initial = client.read_holding_registers(unit, 0, 2).await.unwrap();
    assert_eq!(initial, vec![11, 22]);

    client.write_single_register(unit, 1, 42).await.unwrap();
    assert_eq!(service.holding_register(1).unwrap(), Some(42));

    client
        .write_multiple_coils(unit, 10, &[true, false, true, true])
        .await
        .unwrap();
    let coils = client.read_coils(unit, 10, 4).await.unwrap();
    assert_eq!(coils, vec![true, false, true, true]);

    client
        .mask_write_register(unit, 1, 0xFF00, 0x0012)
        .await
        .unwrap();
    let masked = client.read_holding_registers(unit, 1, 1).await.unwrap();
    assert_eq!(masked, vec![0x0012]);

    let rw = client
        .read_write_multiple_registers(unit, 0, 2, 0, &[0x7777, 0x8888])
        .await
        .unwrap();
    assert_eq!(rw, vec![0x7777, 0x8888]);

    let report = client.report_server_id(unit).await.unwrap();
    assert_eq!(report.server_id, 1);
    assert!(report.run_indicator_status);
    assert!(report.additional_data.is_empty());

    let device_id = client.read_device_identification(unit, 0x01, 0x00).await.unwrap();
